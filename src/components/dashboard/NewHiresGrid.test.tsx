...
describe("NewHiresGrid", () => {
  ...
  it("sorts employees by start date (most recent first)", () => {
    ...
    const employeeCards = screen.getAllByText(
      /Desarrollador Frontend|Diseñadora UX\/UI|Analista de Datos/
    );
  ...
  it("shows empty state when no employees", () => {
    ...
    expect(
      screen.getByText("No hay nuevos empleados para mostrar")
    ).toBeInTheDocument();
  ...
  it("displays employee bio in modal", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "Especialista en React y TypeScript con 3 años de experiencia."
        )
      ).toBeInTheDocument();
    });
  });
  ...
  it("applies custom className", () => {
    ...
    const cardElement = screen
      .getByText("Nuevos Empleados")
      .closest(".custom-grid-class");
