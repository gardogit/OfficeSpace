...
const NewsCarouselComponent: React.FC<NewsCarouselProps> = ({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentIndex((prevIndex) => {
        ...
        if (article) {
          announce(
            `Artículo ${newIndex + 1} de ${news.length}: ${article.title}`,
            "polite"
          );
        }
        return newIndex;
  ...
  const goToSlide = useCallback(
    (index: number) => {
      ...
      if (article) {
        announce(
          `Navegando al artículo ${index + 1}: ${article.title}`,
          "polite"
        );
      }
    },
    ...
    (event: KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case KEYS.SPACE:
          ...
          announce(
            isPaused ? "Carrusel reanudado" : "Carrusel pausado",
            "polite"
          );
          break;
      }
      ...
      <Card className="h-full">
        ...
        >
          ...
          <div className="sr-only">
            Carrusel de noticias. Usa las flechas izquierda y derecha para
            navegar. Presiona espacio para pausar o reanudar la rotación
            automática.
          </div>
          {/* Article image */}
                ...
                >
                  <Button
                    ...
                    aria-label={`Ir al artículo anterior (${
                      currentIndex === 0 ? news.length : currentIndex
                    } de ${news.length})`}
                    iconPosition="left"
                    icon={
                      <svg
                  ...
                  <Button
                    ...
                    aria-label={`Ir al siguiente artículo (${
                      currentIndex + 2 > news.length ? 1 : currentIndex + 2
                    } de ${news.length})`}
                    iconPosition="right"
                    icon={
                      <svg
