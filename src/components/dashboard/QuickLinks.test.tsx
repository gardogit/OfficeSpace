...
describe('QuickLinks', () => {
  ...
  describe('Renderizado básico', () => {
    ...
    it('aplica className personalizado', () => {
      const { container } = render(<QuickLinks links={mockLinks} className="custom-class" />);
  ...
  describe('Iconos y elementos visuales', () => {
    ...
    it('muestra indicador de enlace externo', () => {
      ...
      const externalIcons = screen.getAllByRole('img', { hidden: true });
      // Filtrar solo los SVGs de enlace externo (no los emojis)
      const externalLinkIcons = document.querySelectorAll('svg');
  ...
  describe('Accesibilidad', () => {
    ...
    it('botones tienen etiquetas aria apropiadas', () => {
      ...
      expect(screen.getByLabelText('Abrir Sistema de Tickets en nueva pestaña')).toBeInTheDocument();
    });
  ...
  describe('Estilos y clases CSS', () => {
    ...
    it('headers de categoría tienen estilos apropiados', () => {
      ...
      categoryHeaders.forEach(header => {
        expect(header).toHaveClass('text-sm', 'font-medium', 'text-gray-700', 'uppercase', 'tracking-wide');
      });
    });
    ...
    it('enlaces tienen truncate para texto largo', () => {
      const longTitleLink: QuickLink[] = [
        {
          id: 'long-link',
          title: 'Este es un título muy largo que debería ser truncado para evitar problemas de layout',
          url: 'https://example.com',
          category: 'Test',
          icon: '📝'
        }
      ...
      const linkText = screen.getByText('Este es un título muy largo que debería ser truncado para evitar problemas de layout');
  ...
  describe('Casos edge', () => {
    it('maneja categorías con caracteres especiales', () => {
      ...
      expect(screen.getByText(/categoría con ñ & símbolos!/i)).toBeInTheDocument();
    });
    ...
    it('maneja URLs malformadas sin romper el componente', () => {
      ...
      expect(mockWindowOpen).toHaveBeenCalledWith('not-a-valid-url', '_blank', 'noopener,noreferrer');
    });
